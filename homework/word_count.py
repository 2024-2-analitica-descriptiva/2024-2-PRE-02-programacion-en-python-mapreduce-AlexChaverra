...
import fileinput
import glob # Librería que me permite leer los archivos
import os.path
...
def line_preprocessing(sequence):
    ...
    sequence = [
        (key, value.lower()) #Elimina las mayusculas
        for key, value in sequence
    ]

    sequence = [
        (key, value.strip()) #Elimina el retorno de linea /n
        for key, value in sequence
    ]
    return sequence
...
def mapper(sequence):
    ...
    return [(word, 1) for _, text_line in sequence for word in text_line.split()] # me parte la línea de texto en palabras, el signo _ me permite guardar el ultimo resultado d una operación
...
def shuffle_and_sort(sequence):
    ...
    return sorted(sequence, key=lambda x: x[0]) # Ordenamos la lista, al parametro sorted le estamos enviando una funcion que indica sobre cuál parametro ordeno
...
def reducer(sequence):
    ...
    # Creamos un diccionario y recorremos la secuencia
    diccionario = {}
    for key, value in sequence:
...
# Siempre se crea un archivo success, es una bandera que indica que todo corrio bien y que los resultados estan completos, quiere decir que corrio todo el proceso, porque podria tener varios sistemas escribiendo en esa carpeta, pero aunque esta tenga resultados, no podría saber que todos escribieron que todos hicieron su tarea
